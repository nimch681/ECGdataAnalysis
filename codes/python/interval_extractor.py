...
def average_by_sample(interval,ten=True,fifty=False, all=True):
...
def interval_and_average(record, ten=True,fifty=False, all_avg=True):
    ...
    for i in range(0, len(record.Q_points)):
        ...
        p_q = sub_signal_interval(record.time,int(record.P_points_properites["onset"][0][i]),int(record.S_points_properites["offset"][i]),0)
        P_Q.append(p_q)

        p_q_neg = sub_signal_interval(record.time,int(record.P_points_properites["onset"][1][i]),int(record.S_points_properites["offset"][i]),0)
        P_Q_neg.append(p_q_neg)
        
        p_r = sub_signal_interval(record.time,int(record.P_points_properites["onset"][0][i]),int(record.R_pos_properites["offset"][i]),0)
        P_R.append(p_r)

        p_r_neg = sub_signal_interval(record.time,int(record.P_points_properites["onset"][1][i]),int(record.R_pos_properites["offset"][i]),0)
        P_R_neg.append(p_r_neg) 

        s_t = sub_signal_interval(record.time,int(record.S_points_properites["onset"][i]),int(record.T_points_properites["offset"][0][i]),0)
        S_T.append(s_t)

        s_t_neg = sub_signal_interval(record.time,int(record.S_points_properites["onset"][i]),int(record.T_points_properites["offset"][1][i]),0)
        S_T_neg.append(s_t_neg)

        r_t = sub_signal_interval(record.time,int(record.R_pos_properites["onset"][i]),int(record.T_points_properites["offset"][0][i]),0)
        R_T.append(r_t)
    ...
    return QRS_properties, P_Q_properties, P_Q_neg, P_R_properties, P_R_neg, S_T_properties, S_T_neg
